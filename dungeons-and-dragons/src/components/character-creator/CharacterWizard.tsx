...
function WizardContent() {
	...
	const validateStepRequired = (stepId: string) => {
		const errors: string[] = [];
		if (stepId === "identity") {
			if (!state.identity?.name || state.identity.name.trim().length === 0)
				errors.push("Please enter a character name before continuing.");
		}
		if (stepId === "race-class") {
			if (!state.race || state.race.trim().length === 0)
				errors.push("Please select a race.");
			if (!state.characterClass || state.characterClass.trim().length === 0)
				errors.push("Please select a class.");
		}
		if (stepId === "abilities") {
			const stats = Object.values(state.abilities || {});
			if (stats.length < 6 || stats.some((s) => typeof s !== "number")) {
				errors.push("All six ability scores must be assigned.");
			}
			if (stats.some((s) => s < 8 || s > 17)) {
				errors.push(
					"Ability scores must be between 8 and 17 for level 1 characters.",
				);
			}
		}
		if (stepId === "equipment") {
			...
			if (!hasEquipment) {
				errors.push(
					"Select at least one piece of equipment, weapon, or armor.",
				);
			}
		}
		return errors;
	...
	const handleNext = () => {
		const errors = validateStepRequired(currentStep.id);
		if (errors.length > 0) {
			...
			{stepErrors.length > 0 && (
				<div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4">
					<div className="flex items-center gap-2 mb-2">
						...
						<h3 className="font-semibold text-red-500">Required Fields</h3>
					</div>
					<ul className="list-disc list-inside space-y-1 pl-7">
						{stepErrors.map((err) => (
							<li key={err} className="text-sm text-red-200/80">
								{err}
							</li>
						))}
					</ul>
