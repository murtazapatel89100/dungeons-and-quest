...
import { ScrollText, Beer, Map, User } from "lucide-react";
...
export function NameGenerator() {
  ...
  const generateNames = (type: GeneratorType) => {
    ...
    <section className="relative w-full py-16">
      <div className="text-center mb-12">
        ...
        <p className="text-[#9CA3AF] max-w-2xl mx-auto">
          Stuck for a name? Generate highly thematic taverns, towns, and NPCs on the fly.
        </p>
      </div>
        ...
        <div className="md:w-64 bg-[#0B0F1A] p-6 border-r border-[#1F2937] flex flex-col gap-4">
          <button 
            ...
            className={`p-4 rounded-xl text-left font-['Cinzel'] transition-all flex items-center gap-3 ${
              activeTab === "tavern" ? "bg-[#3B82F6]/10 text-[#60A5FA] border border-[#3B82F6]/50 shadow-[0_0_15px_rgba(59,130,246,0.1)]" : "text-[#9CA3AF] hover:bg-[#1F2937] border border-transparent"
            }`}
          >
          ...
          <button 
            ...
            className={`p-4 rounded-xl text-left font-['Cinzel'] transition-all flex items-center gap-3 ${
              activeTab === "npc" ? "bg-[#3B82F6]/10 text-[#60A5FA] border border-[#3B82F6]/50 shadow-[0_0_15px_rgba(59,130,246,0.1)]" : "text-[#9CA3AF] hover:bg-[#1F2937] border border-transparent"
            }`}
          >
          ...
          <button 
            ...
            className={`p-4 rounded-xl text-left font-['Cinzel'] transition-all flex items-center gap-3 ${
              activeTab === "town" ? "bg-[#3B82F6]/10 text-[#60A5FA] border border-[#3B82F6]/50 shadow-[0_0_15px_rgba(59,130,246,0.1)]" : "text-[#9CA3AF] hover:bg-[#1F2937] border border-transparent"
            }`}
          >
            <Map className="w-5 h-5" /> Towns
          </button>
        </div>
          ...
          <div className="flex-1 flex flex-col gap-3">
            <AnimatePresence mode="popLayout">
              {generatedNames.map((name, idx) => (
                ...
                >
                  <span className="text-[#60A5FA] font-['Cinzel'] text-xl tracking-wide">{name}</span>
                </motion.div>
              ))}
            </AnimatePresence>
          </div>
