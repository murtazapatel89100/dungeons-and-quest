...
import { EyeOff, HeartHandshake, Ghost, Link, Zap, Skull, ArrowDownToLine, Lock, CircleHelp } from "lucide-react";
import { rulesData } from "@/lib/data/rulesData";

const getConditionIcon = (iconName: string) => {
  const icons: Record<string, any> = {
    EyeOff, HeartHandshake, Ghost, Link, Zap, Skull, ArrowDownToLine, Lock
  };
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 px-4">
        {rulesData.conditions.map((condition, idx) => {
          ...
          return (
            ...
            >
              <div 
                onClick={() => setActiveConditionId(isActive ? null : condition.id)}
              ...
              >
                ...
                <div className={`p-6 flex flex-col items-center justify-center text-center transition-colors ${isActive ? "bg-[#1F2937]/50" : ""}`}>
                  <div className={`p-4 rounded-full mb-4 bg-[#0B0F1A] border transition-colors duration-300 ${isActive ? condition.color : "border-[#374151] group-hover:" + condition.color} text-[#D1D5DB]`}>
                    {getConditionIcon(condition.icon)}
                  </div>
                  <h3 className="font-['Cinzel'] text-xl text-[#F9FAFB]">{condition.name}</h3>
                </div>
                      ...
                      <div className="pt-4 border-t border-[#374151]">
                        ...
                        <ul className="space-y-2">
                          {condition.effects.map((effect, eIdx) => (
                            <li key={eIdx} className="text-sm text-[#D1D5DB] flex items-start gap-2">
                              <span className="text-[#EC4899] mt-1 text-xs">◆</span>
                              <span>{effect}</span>
                            </li>
                          ))}
                        </ul>
