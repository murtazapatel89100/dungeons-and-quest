...
import { Eye, Flame, MountainSnow } from "lucide-react";
import { AnimatePresence, motion } from "motion/react";
import { useState } from "react";
import { rulesData } from "@/lib/data/rulesData";

export function EnvironmentRules() {
  const [activeTab, setActiveTab] = useState<"resting" | "vision" | "hazards">(
    "resting",
  );
    ...
    <section className="relative w-full py-16">
      <div className="text-center mb-12">
        ...
        <p className="text-[#9CA3AF] max-w-2xl mx-auto">
          The world is a dangerous place. Surviving means knowing when to rest,
          how to see in the dark, and how to avoid deadly hazards.
        </p>
      </div>
              ...
              {activeTab === "vision" && (
                <div>
                  ...
                  <div className="space-y-4">
                    {vision.categories.map((cat) => (
                      <div
                        key={cat.name}
                        className="bg-[#0B0F1A] border border-[#1F2937] p-5 rounded-lg flex flex-col sm:flex-row sm:items-center gap-4"
                      >
                        <div className="w-1/3 shrink-0">
                          <h4 className="font-bold text-[#F9FAFB] text-lg">
                            {cat.name}
                          </h4>
                        </div>
                        <div className="w-2/3">
